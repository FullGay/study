...
　
  ...
  var calcArea_x = 19200;
  var calcArea_y = 19200;
  var unit = "m";
  ctx.font = "12px 'Arial'";			//フォント指定
  ctx.fillStyle = "rgb(0,0,0)";		//色指定
    var delta_x_axis = size.x/(2**map.getZoom())/axisNum;//何ピクセル毎にx軸を描くか
    var delta_y_axis = size.y/(2**map.getZoom())/axisNum;//何ピクセル毎にy軸を描くか
  ...
  for(var i = getGoodPlace_X(delta_x_axis);i<Math.floor(bounds.getEast());i+=delta_x_axis){
    ...
    ctx.fillText(num*calcArea_x/size.x+"("+unit+")",2**map.getZoom()*(i-bounds.getWest())-5+100,window.innerHeight-80);//x軸の値表示
  }
  ///////////////draw Y axis///////////////
  for(var i = getGoodPlace_Y(delta_y_axis);i<Math.floor(bounds.getNorth());i+=delta_y_axis){
    ctx.fillRect(90, 2**map.getZoom()*(-i+bounds.getNorth())-1, 10, 2);//y軸のめもり表示
    var num = Math.abs(i % size.y);
    if(i<0 && num!=0){num = size.y - num;}
    ctx.fillText(num*calcArea_y/size.y+"("+unit+")", 55, 2**map.getZoom()*(-i+bounds.getNorth())-5);//x軸の値表示  55は左端からの幅
  }
