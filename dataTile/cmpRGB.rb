        ...
        (2**z).times do |y|
            ...
            File.open("#{y}_pre.txt","w") do |fw|
                ...
                File.open("tmp.txt"){|f|
                    f.gets()
                    f.each_line{|line|
                    if /grey|silver|DarkKhaki|turquoise|RosyBrown/=~line
                      ...
                      b = Integer("0x#{tmp}")
                      line.gsub!(/grey.*|silver.*|DarkKhaki.*|turquoise.*|RosyBrown.*/,"srgb(#{r},#{g},#{b})")
                    end
                    #begin if /turquoise/=~line
                    #    tmp = line.slice(line.index("#")+1,2)
                    #    r = Integer("0x#{tmp}")
                    #    tmp = line.slice(line.index("#")+3,2)
                    #    g = Integer("0x#{tmp}")
                    #    tmp = line.slice(line.index("#")+5,2)
                    #    b = Integer("0x#{tmp}")
                    #    line.gsub!(/turquoise./,"srgb(#{r},#{g},#{b})")
                    #end
                        line.slice!(/\(.*srgb/)
                        fw.puts(line)
                    }
